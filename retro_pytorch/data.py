...
def split_into_chunks(seq_tokens, seq_length, pad_id=0):
    ...
    if remainder > 0:
        last_chunk = np.pad(seq_tokens[num_chunks * seq_length:], (pad_id, seq_length - remainder), mode='constant')
            ...
            for line in reader:
                ...
                for seq in seq_chunks:   
                    ...
                    if len(seq_tokens) < self.seq_length + 1:
                        seq_tokens = F.pad(seq_tokens, (self.pad_id, self.seq_length + 1 - len(seq_tokens)))
