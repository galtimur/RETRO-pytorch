...
from retro_pytorch.retrieval import text_folder_to_chunks_, chunks_to_precalculated_knn_, bert_embed, SOS_ID, EOS_ID, PAD_TOKEN
...
class TrainingWrapper(nn.Module):
    ...
    def fetch_neighbours(self, seq, doc_except):
        ...
        distances, indices = self.fetch_knn_chunks_check_fn(past_seq_chunks, doc_except=doc_except, knn = total_neighbors_to_fetch, isdecoder = True)
               
        
        with  memmap(self.chunks_memmap_path, dtype = np.int32, shape = (self.num_chunks + 1, self.chunk_size + 1), mode='r') as chunk_memmap  \
            , memmap(self.doc_ids_memmap_path, shape=(self.num_chunks,), dtype=np.int32, mode='r') as doc_ids_storage:
